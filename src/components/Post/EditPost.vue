...
const props = defineProps({
  postId: {
   ...
   post: {
     type: String, // since we are using JSON.stringify()
      required: true
   }
});

const state = reactive({});
state.post = (props?.post) ? JSON.parse(props.post) : {};
...
const submit = () => {
  ...
  PostRepository.update(state.post, state.post.id);
}
</script>
